...
cost ..              zc=e=sum(i,y(i)+c_bar(i)*u_i(i)+h_bar(i)*v_i(i)) ;
*cost ..              zc=e=sum(i,y(i)+c_bar(i)*u_i(i)+h_bar(i)*v_i(i)+yy(i)) + sumV ;
demEq ..             sum(i$(ord(i) ne %subprob%),z(i))-z('%subprob%')=e=d;
PWLEq1(i) ..         sum(dynk(i,k),g(i,k)) =e= u_i(i);
PWLEq2(i) ..         z(i) =e= sum(dynk(i,k),q(i,k)*g(i,k));
PWLEq3(i) ..         y(i) =e= sum(dynk(i,k),c(i,k)*g(i,k));
*PWLEq11(i)$((ord(i) le numGens) and (fv_i(i) eq 1)) .. sum(dynikk(i,kk),gg(i,kk)) =e= 1;
*PWLEq22(i)$((ord(i) le numGens) and (fv_i(i) eq 1)) .. z(i) =e= sum(dynikk(i,kk),Plevels(i,kk)*gg(i,kk));
*PWLEq33(i)$((ord(i) le numGens) and (fv_i(i) eq 1)) .. yy(i) =e= sum(dynikk(i,kk),fV(i,kk)*gg(i,kk));

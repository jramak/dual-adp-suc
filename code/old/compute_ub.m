function [ub_cost,ub_time] = compute_ub(filename)
...
%		end
        ...
        for i = find(fv_i)'
            tmp = find(z_lo(i) <= Plevels(:,i));
            assert(~isempty(tmp));
%             z_idx_lo(i) = max(tmp(1)-1,1);
            z_idx_lo(i) = tmp(1);
            tmp = find(Plevels(:,i) <= z_hi(i));
            assert(~isempty(tmp));
%             z_idx_hi(i) = min(tmp(end)+1,nPts);
        ...
        [status,result]=system(['gams subgrad_ub_cont --probdata=',...

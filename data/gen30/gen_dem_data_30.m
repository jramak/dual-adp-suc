function gen_dem_data_30(maxDemand,percent_sig)

numOffers = 10;
numOffset = 15; % 15 generator already complete
numGens = 30;
...
price = r_price((numOffers+numOffset+1):(numOffers+numOffset+numGens),1:numOffers);

s2.name = 'GenOfferQty';
s2.form = 'full';
r_struct = rgdx('uc_all',s2);
r_qty = r_struct.val;
qty = r_qty((numOffers+numOffset+1):(numOffers+numOffset+numGens),1:numOffers);
...
h_bar = [r_h_bar((numOffers+numOffset+1):(numOffers+numOffset+numGens));0;0];
...
c_bar = [r_c_bar((numOffers+numOffset+1):(numOffers+numOffset+numGens));0;0];
...
Ld = [r_minDownTime((numOffers+numOffset+1):(numOffers+numOffset+numGens));1;1];
...
Lu = [r_minUpTime((numOffers+numOffset+1):(numOffers+numOffset+numGens));1;1];

s7.name = 'xmax';
s7.form = 'full';
r_struct = rgdx('uc_all',s7);
r_qmax = r_struct.val;
qmax = r_qmax((numOffers+numOffset+1):(numOffers+numOffset+numGens));

s8.name = 'xmin';
s8.form = 'full';
r_struct = rgdx('uc_all',s8);
r_qmin = r_struct.val;
qmin = r_qmin((numOffers+numOffset+1):(numOffers+numOffset+numGens));
...
iwgdx(['uc30_',num2str(maxDemand),'md_',num2str(percent_sig*100),...
